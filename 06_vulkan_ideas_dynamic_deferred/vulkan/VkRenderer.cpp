...
bool VkRenderer::init(unsigned int width, unsigned int height) {
  ...
  updateFramebufferAttachmentDescriptorSets();
...
bool VkRenderer::createDescriptorLayouts() {
...

  {
    ...
    result = vkCreateDescriptorSetLayout(mRenderData.rdVkbDevice.device, &assimpCreateInfo,
      nullptr, &mRenderData.rdGroundMeshDescriptorLayout);
    if (result != VK_SUCCESS) {
...

  {
    ...
    result = vkCreateDescriptorSetLayout(mRenderData.rdVkbDevice.device, &assimpSkyboxCreateInfo,
      nullptr, &mRenderData.rdSkyboxDescriptorLayout);
    if (result != VK_SUCCESS) {
...

  {
    ...
    std::vector<VkDescriptorSetLayoutBinding> layoutBindings = { assimpUboBind, colorBinding, positionBinding, normalBinding, selectionBinding };

    VkDescriptorSetLayoutCreateInfo inputLayoutInfo{};
    inputLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    inputLayoutInfo.bindingCount = static_cast<uint32_t>(layoutBindings.size());
    inputLayoutInfo.pBindings = layoutBindings.data();

    if (vkCreateDescriptorSetLayout(mRenderData.rdVkbDevice.device, &inputLayoutInfo,
        nullptr, &mRenderData.rdCompositeDescriptorLayout) != VK_SUCCESS) {
...
void VkRenderer::cleanupDepthBuffer() {
...
bool VkRenderer::createGBuffer() {
  ...
  if (!FramebufferAttachment::init(mRenderData, mRenderData.gBuffer.position, VK_FORMAT_R16G16B16A16_SFLOAT,
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT)) {
    return false;
  }
  Logger::log(1, "%s: create GBuffer normal attachment (4x 32 bit float)\n", __FUNCTION__);
  if (!FramebufferAttachment::init(mRenderData, mRenderData.gBuffer.normal, VK_FORMAT_R16G16B16A16_SFLOAT,
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT)) {
    return false;
  }
...
bool VkRenderer::createSelectionImage() {
  ...
  selecImageInfo.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
  ...
  if (vkCreateSampler(mRenderData.rdVkbDevice.device, &samplerInfo, nullptr, &mRenderData.rdSelectionImageSampler) != VK_SUCCESS) {
...
void VkRenderer::cleanupSelectionImage() {
  vkDestroySampler(mRenderData.rdVkbDevice.device, mRenderData.rdSelectionImageSampler, nullptr);
...
bool VkRenderer::createPipelineLayouts() {
  ...
  std::vector<VkDescriptorSetLayout> groundMeshLayouts = {
    mRenderData.rdGroundMeshDescriptorLayout };

  if (!PipelineLayout::init(mRenderData, mRenderData.rdGroundMeshPipelineLayout, groundMeshLayouts)) {
  ...
  std::vector<VkDescriptorSetLayout> skyboxLayouts = {
    mRenderData.rdAssimpTextureDescriptorLayout,
    mRenderData.rdSkyboxDescriptorLayout };

  if (!PipelineLayout::init(mRenderData, mRenderData.rdSkyboxPipelineLayout, skyboxLayouts)) {
    Logger::log(1, "%s error: could not init Assimp skybox pipeline layout\n", __FUNCTION__);
    return false;
  }

  std::vector<VkDescriptorSetLayout> compositeLayout = {
    mRenderData.rdCompositeDescriptorLayout };

  if (!PipelineLayout::init(mRenderData, mRenderData.rdCompositePipelineLayout, compositeLayout)) {
...
bool VkRenderer::createPipelines() {
  ...
  if (!SkinningPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdAssimpSkinningPipelineLayout,
  ...
  if (!SkinningPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdAssimpSelectionPipelineLayout,
      mRenderData.rdAssimpSelectionPipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!SkinningPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdAssimpSkinningSelectionPipelineLayout,
      mRenderData.rdAssimpSkinningSelectionPipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!SkinningPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdAssimpSkinningMorphPipelineLayout,
      mRenderData.rdAssimpSkinningMorphPipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!SkinningPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdAssimpSkinningMorphSelectionPipelineLayout,
  ...
  if (!SkinningPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdAssimpLevelPipelineLayout,
      mRenderData.rdAssimpLevelPipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!GroundMeshPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdLinePipelineLayout,
      mRenderData.rdGroundMeshPipeline,
      vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!LinePipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdLinePipelineLayout,
      mRenderData.rdLinePipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!LinePipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdSpherePipelineLayout,
      mRenderData.rdSpherePipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!SkyboxPipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdSkyboxPipelineLayout,
      mRenderData.rdSkyboxPipeline, vertexShaderFile, fragmentShaderFile)) {
  ...
  if (!CompositePipeline::init(mRenderData, colorAttachmentFormats, mRenderData.rdCompositePipelineLayout,
    mRenderData.rdCompositePipeline, vertexShaderFile, fragmentShaderFile)) {
...
bool VkRenderer::draw(float deltaTime) {
  ...
  glm::vec4 lightPos;
  lightPos.x = std::cos(glm::radians(mRenderData.rdLightSourceAngleEastWest));
  ...
  lightPos.z = std::cos(glm::radians(mRenderData.rdLightSourceAngleNorthSouth));
  lightPos.w = 0.0f; // alpha, ignored
  ...
  glm::mat4 lightMatrix = glm::mat4(lightPos, lightColor, glm::vec4(mRenderData.rdFogDensity, glm::vec3()), glm::vec4());
  mMatrices.lightAndFogMatrix = lightMatrix;
  ...
  normalClearValue.color = { { 0.0, 0.0, 0.0, 0.0f } };
  ...
  firstImageMemoryBarrier.image = mRenderData.rdSelectionImage;
  vkCmdPipelineBarrier(
    mRenderData.rdCommandBuffer,
    VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,  // srcStageMask
    VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, // dstStageMask
  ...
  positionAttachmentInfo.clearValue = positionClearValue;
  ...
  selectionAttachmentInfo.imageView = mRenderData.rdSelectionImageView;
  ...
  depthAttachmentInfo.imageView = mRenderData.rdDepthImageView;
  ...
  /* Composite pass */
  vkCmdBindPipeline(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mRenderData.rdCompositePipeline);
  vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mRenderData.rdCompositePipelineLayout, 0, 1,
    &mRenderData.rdCompositeDescriptorSet, 0, nullptr);

  vkCmdDraw(mRenderData.rdCommandBuffer, 3, 1, 0, 0);
  ...
  if (mRenderData.rdDrawSkybox) {
    ...
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mSkyboxBuffer.buffer, &offset);

    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mSphereModel.getVertexData().vertices.size()), 1, 0, 0);
  }
  ...
  if (mLineIndexCount > 0) {
    ...
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mLineVertexBuffer.buffer, &offset);
    vkCmdSetLineWidth(mRenderData.rdCommandBuffer, 3.0f);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mLineMesh->vertices.size()), 1, 0, 0);
  }


  /* draw colliding spheres */
  if (mCollidingSphereCount > 0) {
    vkCmdBindPipeline(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mRenderData.rdSpherePipeline);

    vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
      mRenderData.rdSpherePipelineLayout, 0, 1, &mRenderData.rdSphereDescriptorSet, 0, nullptr);

    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mSphereVertexBuffer.buffer, &offset);
    vkCmdSetLineWidth(mRenderData.rdCommandBuffer, 3.0f);
    vkCmdDraw(mRenderData.rdCommandBuffer, sphereVertexCount, mCollidingSphereCount, 0, 0);
  }
  ...
  if (mRenderData.rdDrawLevelAABB && !mLevelAABBMesh->vertices.empty()) {
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mLevelAABBVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mLevelAABBMesh->vertices.size()), 1, 0, 0);
  }

  if (mRenderData.rdDrawLevelWireframe && !mLevelWireframeMesh->vertices.empty()) {
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mLevelWireframeVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mLevelWireframeMesh->vertices.size()), 1, 0, 0);
  }

  if (mRenderData.rdDrawLevelOctree && !mLevelOctreeMesh->vertices.empty()) {
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mLevelOctreeVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mLevelOctreeMesh->vertices.size()), 1, 0, 0);
  }

  if (mRenderData.rdDrawIKDebugLines && !mIKFootPointMesh->vertices.empty()) {
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mIKLinesVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mIKFootPointMesh->vertices.size()), 1, 0, 0);
  }
  mRenderData.rdLevelCollisionTime += mLevelCollisionTimer.stop();

  if (mRenderData.rdDrawInstancePaths && !mInstancePathMesh->vertices.empty()) {
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mInstancePathVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mInstancePathMesh->vertices.size()), 1, 0, 0);
  }

  if (mRenderData.rdDrawNeighborTriangles && !mLevelGroundNeighborsMesh->vertices.empty()) {
    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mGroundMeshNeighborVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, static_cast<uint32_t>(mLevelGroundNeighborsMesh->vertices.size()), 1, 0, 0);
  }

  mLevelGroundNeighborUpdateTimer.start();
  if (mRenderData.rdDrawGroundTriangles) {
    vkCmdBindPipeline(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mRenderData.rdGroundMeshPipeline);

    vkCmdBindDescriptorSets(mRenderData.rdCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
      mRenderData.rdLinePipelineLayout, 0, 1, &mRenderData.rdGroundMeshDescriptorSet, 0, nullptr);

    VkDeviceSize offset = 0;
    vkCmdBindVertexBuffers(mRenderData.rdCommandBuffer, 0, 1, &mGroundMeshVertexBuffer.buffer, &offset);
    vkCmdDraw(mRenderData.rdCommandBuffer, mGroundMeshVertexCount, 1, 0, 0);
  }
  mRenderData.rdLevelGroundNeighborUpdateTime += mLevelGroundNeighborUpdateTimer.stop();
  ...
  vkCmdBeginRendering(mRenderData.rdCommandBuffer, &uiRenderInfo);
  ...
  if (mRenderData.rdApplicationMode == appMode::edit) {
    if (mMousePick) {
      ...
      float selectedInstanceId = getPixelValueFromPos(mRenderData, mRenderData.rdSelectionImage, mMouseXPos, mMouseYPos);
...
void VkRenderer::cleanup() {
  ...
  PipelineLayout::cleanup(mRenderData, mRenderData.rdGroundMeshPipelineLayout);
  PipelineLayout::cleanup(mRenderData, mRenderData.rdSkyboxPipelineLayout);
  ...
  vkDestroyDescriptorSetLayout(mRenderData.rdVkbDevice.device, mRenderData.rdGroundMeshDescriptorLayout, nullptr);
  vkDestroyDescriptorSetLayout(mRenderData.rdVkbDevice.device, mRenderData.rdSkyboxDescriptorLayout, nullptr);
  vkDestroyDescriptorSetLayout(mRenderData.rdVkbDevice.device, mRenderData.rdCompositeDescriptorLayout, nullptr);
