...
bool AssimpModel::loadModel(VkRenderData &renderData, std::string modelFilename, unsigned int extraImportFlags) {
  ...
  for (const auto& mesh : mModelMeshes) {
    ...
    VertexBuffer::uploadData(renderData, vertexBuffer, mesh.vertices);
...
void AssimpModel::setPhysicsEnabled(bool value) {
  mModelSettings.msPhysicsEnabled = value;
}

bool AssimpModel::isPhysicsEnabled() {
  return mModelSettings.msPhysicsEnabled;
}

physicsHullType AssimpModel::getPhysicsHulLType() {
  return mModelSettings.msPhysicsHullType;
}

void AssimpModel::setPhysicsHullType(physicsHullType hulLType) {
  mModelSettings.msPhysicsHullType = hulLType;
}
