...
bool UserInterface::init(VkRenderData& renderData) {
  ...
  imguiIinitInfo.PipelineInfoMain.PipelineRenderingCreateInfo.depthAttachmentFormat = renderData.rdDepthBufferData.format;
  ...
  mPhysicsValues.resize(mNumPhysicsValues);
...
void UserInterface::createSettingsWindow(VkRenderData& renderData, ModelInstanceCamData& modInstCamData) {
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("View")) {
      ...
      ImGui::MenuItem("Shader Debug", nullptr, &mDebugWindowOpen);
      ImGui::MenuItem("Status Bar", nullptr, &mStatusBarVisible);
      ImGui::MenuItem("Grid", nullptr, &renderData.rdEnableInfiniteGrid);
      ImGui::EndMenu();
    }
  ...
  while (mUpdateTime < ImGui::GetTime()) {
    ...
    mPhysicsValues.at(mPhysicsOffset) = renderData.rdPhysicsTime;
    mPhysicsOffset = ++mPhysicsOffset % mNumPhysicsValues;
  ...
  if (ImGui::CollapsingHeader("Timers")) {
    ...
    ImGui::Text("Physics Simulation:      %10.4f ms", renderData.rdPhysicsTime);

    if (ImGui::IsItemHovered()) {
      ImGui::BeginTooltip();
      float averagePhysics = 0.0f;
      for (const auto value : mPhysicsValues) {
        averagePhysics += value;
      }
      averagePhysics /= static_cast<float>(mNumPhysicsValues);
      std::string physicsOverlay = "now:     " + std::to_string(renderData.rdPhysicsTime) +
        " ms\n30s avg: " + std::to_string(averagePhysics) + " ms";
      ImGui::Text("Physics Simulation");
      ImGui::SameLine();
      ImGui::PlotLines("##PhysicsSim", mPhysicsValues.data(), mPhysicsValues.size(), mPhysicsOffset,
        physicsOverlay.c_str(), 0.0f, std::numeric_limits<float>::max(), ImVec2(0, 80));
      ImGui::EndTooltip();
    }
  }
  ...
  if (ImGui::CollapsingHeader("Physics")) {
    ...
    ImGui::SameLine();
    if (ImGui::Button("Update Physics Objects")) {
      modInstCamData.micBakePhyiscsDataCallbackFunction();
    }
  ...
  if (ImGui::CollapsingHeader("Deferred Shading")) {
    ...
    ImGui::SameLine();
    if (ImGui::RadioButton("Position##CompositePosition",
  ...
  if (ImGui::CollapsingHeader("Shadow Map")) {
    ImGui::Text("Enable Shadow Map:");
    ImGui::SameLine();
    ImGui::Checkbox("##EnableShadowMap", &renderData.rdEnableShadowMap);

    if (!renderData.rdEnableShadowMap) {
      ImGui::BeginDisabled();
    }

    ImGui::AlignTextToFramePadding();
    ImGui::Text("Shadow Map Split: ");
    ImGui::SameLine();
    ImGui::SliderFloat("##ShadowMapSplit", &renderData.rdShadowMapSplitLambda, 0.01f, 1.0f, "%.3f", flags);

    ImGui::AlignTextToFramePadding();
    ImGui::Text("Const. Depth Bias:");
    ImGui::SameLine();
    ImGui::SliderFloat("##ShadowMapConstantDepthBias", &renderData.rdShadowMapConstantDepthBias, 0.0f, 5.0f, "%.6f", flags);

    ImGui::AlignTextToFramePadding();
    ImGui::Text("Slope Depth Bias: ");
    ImGui::SameLine();
    ImGui::SliderFloat("##ShadowMapSlopeDepthBias", &renderData.rdShadowMapSlopeDepthBias, 0.0f, 5.0f, "%.6f", flags);

    ImGui::AlignTextToFramePadding();
    ImGui::Text("Enable PCF Filter:");
    ImGui::SameLine();
    ImGui::Checkbox("##EnableShadowMapPCF", &renderData.rdEnableShadowMapPCF);

    if (!renderData.rdEnableShadowMapPCF) {
      ImGui::BeginDisabled();
    }

    ImGui::AlignTextToFramePadding();
    ImGui::Text("PCF Filter Scale: ");
    ImGui::SameLine();
    ImGui::SliderFloat("##ShadowMapPCFScale", &renderData.rdShadowMapPCFScale, 0.25, 2.0, "%f", flags);

    ImGui::AlignTextToFramePadding();
    ImGui::Text("PCF Filter Range: ");
    ImGui::SameLine();
    ImGui::SliderInt("##ShadowMapPCFRange", &renderData.rdShadowMapPCFRange, 1, 4, "%d", flags);
  ...
  if (ImGui::CollapsingHeader("Dynamic Light")) {
    ...
    ImGui::SameLine();
    ImGui::SliderFloat3("##DynLightPos", glm::value_ptr(lightSettings.dlsWorldPosition),
      -250.0f, 250.0f, "%.3f", flags);

    ImGui::AlignTextToFramePadding();
    ImGui::Text("Rotation (X/Y/Z): ");
    ImGui::SameLine();
    ImGui::SliderFloat3("##DynLightRot", glm::value_ptr(lightSettings.dlsWorldRotation),
      -180.0f, 180.0f, "%.3f", flags);

    ImGui::AlignTextToFramePadding();
    ImGui::Text("RGB Light Color:  ");
    ImGui::SameLine();
    ImGui::SliderFloat3("##DynLightightCol", glm::value_ptr(lightSettings.dlsDiffuseColor), 0.0f, 1.0f, "%.3f", flags);
    ...
    if (ImGui::Button("Reset Values##DynLight")) {
      ...
      int instanceIndex = lightSettings.dlsIndexPosition;
  ...
  if (ImGui::CollapsingHeader("Models")) {
    ...
    if (!modelListEmtpy) {
      ...
      modelSettings = selectedModel->getModelSettings();
    ...
    if (ImGui::Button("Create New Instance")) {
      modInstCamData.micInstanceAddCallbackFunction(selectedModel);
    }
    ...
    if (ImGui::Button("Go!##Create")) {
      modInstCamData.micInstanceAddManyCallbackFunction(selectedModel, mManyInstanceCreateNum);
    }
    ...
    ImGui::SameLine();
    ImGui::Checkbox("##ModelIsNavTarget", &modelSettings.msUseAsNavigationTarget);
    if (modelListEmtpy) {
      ImGui::EndDisabled();
    }

    if (selectedModel) {
      selectedModel->setModelSettings(modelSettings);
    }
  }

  renderData.rdDrawModelPhysicsBodies = false;
  if (ImGui::CollapsingHeader("Models Physics")) {
    bool modelListEmtpy = modInstCamData.micModelList.size() == 1;
    ...
    if (!modelListEmtpy) {
      ...
      ImGui::SameLine();
      ImGui::Checkbox("##EnablePhysics", &modelSettings.msPhysicsEnabled);
      if (modelListEmtpy) {
        ImGui::EndDisabled();
      }

      if (modelSettings.msPhysicsEnabled) {
        renderData.rdDrawModelPhysicsBodies = true;

        // map with hull ytpes
        std::string selectedHullType = "None";
        std::string selectedBodyTime = "None";

        ImGui::AlignTextToFramePadding();
        ImGui::Text("Body Type:         ");
        ImGui::SameLine();
        ImGui::PushItemWidth(100.0f);
        if (ImGui::BeginCombo("##PhysicsBodyType",
          modInstCamData.micPhysicsBodyTypeMap.at(modelSettings.msPhysicsBodyType).c_str())) {
          for (int i = 0; i < modInstCamData.micPhysicsBodyTypeMap.size(); ++i) {
            if (modInstCamData.micPhysicsBodyTypeMap[static_cast<physicsBodyType>(i)].empty()) {
              continue;
            }
            const bool isSelected = (static_cast<int>(modelSettings.msPhysicsBodyType) == i);
            if (ImGui::Selectable(modInstCamData.micPhysicsBodyTypeMap[static_cast<physicsBodyType>(i)].c_str(), isSelected)) {
              modelSettings.msPhysicsBodyType = static_cast<physicsBodyType>(i);
            }
        ...
        ImGui::SameLine();
        ImGui::PushItemWidth(100.0f);
        if (ImGui::BeginCombo("##PhysicsHullType",
          modInstCamData.micPhysicsHullTypeMap.at(modelSettings.msPhysicsHullType).c_str())) {
          for (int i = 0; i < modInstCamData.micPhysicsHullTypeMap.size(); ++i) {
            if (modInstCamData.micPhysicsHullTypeMap[static_cast<physicsHullType>(i)].empty()) {
              continue;
            }
            const bool isSelected = (static_cast<int>(modelSettings.msPhysicsHullType) == i);
            if (ImGui::Selectable(modInstCamData.micPhysicsHullTypeMap[static_cast<physicsHullType>(i)].c_str(), isSelected)) {
              modelSettings.msPhysicsHullType = static_cast<physicsHullType>(i);

              ModelSettings defaultSettings;
              modelSettings.msPhysicsHullOffset = defaultSettings.msPhysicsHullOffset;
              modelSettings.msPhysicsHullRotation = defaultSettings.msPhysicsHullRotation;
              modelSettings.msPhysicsHullScale = defaultSettings.msPhysicsHullScale;
            }
        ...
        if (modelSettings.msPhysicsHullType == physicsHullType::box) {
          ...
          ImGui::SameLine();
          ImGui::SliderFloat3("##PhysicsHullSize", glm::value_ptr(modelSettings.msPhysicsHullSize),
            0.1f, 20.0f, "%.3f", flags);
        } else {
          float physicsHullSize = modelSettings.msPhysicsHullSize.x;
          ImGui::AlignTextToFramePadding();
          ImGui::Text("Size (X/Y/Z):      ");
          ImGui::SameLine();
          ImGui::SliderFloat("##PhysicsHullSize", &physicsHullSize,
            0.1f, 20.0f, "%.4f", flags);
          if (ImGui::IsItemDeactivatedAfterEdit() || ImGui::IsItemActive()) {
            modelSettings.msPhysicsHullSize = glm::vec3(physicsHullSize);
          }
        }
        ...
        if (modelSettings.msPhysicsHullType == physicsHullType::box) {
          ...
          ImGui::Text("Scale:             ");
          ImGui::SameLine();
          ImGui::SliderFloat3("##PhysicsHullScale", glm::value_ptr(modelSettings.msPhysicsHullScale),
            0.001f, 10.0f, "%.3f", flags);
        }
        else{
          float physicsHullScale = modelSettings.msPhysicsHullScale.x;
          ImGui::AlignTextToFramePadding();
          ImGui::Text("Scale:             ");
          ImGui::SameLine();
          ImGui::SliderFloat("##PhysicsHullScale", &physicsHullScale,
            0.001f, 10.0f, "%.4f", flags);
          if (ImGui::IsItemDeactivatedAfterEdit() || ImGui::IsItemActive()) {
            modelSettings.msPhysicsHullScale = glm::vec3(physicsHullScale);
          }
        }

        ImGui::Text("                   ");
        ImGui::SameLine();
        if (ImGui::Button("Reset Values to Defaults##PhysicsHull")) {
          ModelSettings defaultSettings;
          modelSettings.msPhysicsHullOffset = defaultSettings.msPhysicsHullOffset;
          modelSettings.msPhysicsHullRotation = defaultSettings.msPhysicsHullRotation;
          modelSettings.msPhysicsHullScale = defaultSettings.msPhysicsHullScale;
        ...
        ImGui::AlignTextToFramePadding();
        ImGui::Text("Dry Friction Coeff:");
        ImGui::SameLine();
        ImGui::SliderFloat("##PhysicsFriction", &modelSettings.msPhysicsFrictionCoeff,
          0.0f, 1.0f, "%.4f", flags);

        ImGui::AlignTextToFramePadding();
        ImGui::Text("Restitution Coeff: ");
        ImGui::SameLine();
        ImGui::SliderFloat("##PhysicsRestitution", &modelSettings.msPhysicsRestitutionCoeff,
          0.0f, 1.0f, "%.4f", flags);
      }
  ...
  if (ImGui::CollapsingHeader("Model Idle/Walk/Run Blendings")) {
    ...
    if (numberOfInstances > 0 && numberOfClips > 0) {
      ...
      // forwarding while erasing
      for (auto iter = modSettings.msIWRBlendings.begin(); iter != modSettings.msIWRBlendings.end(); ) {
  ...
  if (ImGui::CollapsingHeader("Model Animation Mappings")) {
    ...
    if (numberOfInstances > 0 && numberOfClips > 0) {
      ...
      unsigned int buttonId = 0;
      // forwarding while erasing
      for (auto iter = modSettings.msActionClipMappings.begin(); iter != modSettings.msActionClipMappings.end(); ) {
  ...
  if (ImGui::CollapsingHeader("Model Allowed Clip Orders")) {
    ...
    if (numberOfInstances > 0 && numberOfClips > 0) {
      ...
      unsigned int buttonId = 0;
      // forwarding while erasing
      for (auto iter = modSettings.msAllowedStateOrder.begin(); iter != modSettings.msAllowedStateOrder.end(); ) {
  ...
  if (ImGui::CollapsingHeader("Node Tree Template")) {
    ...
    // forwarding while erasing
    for (auto iter = modInstCamData.micBehaviorData.begin(); iter != modInstCamData.micBehaviorData.end();) {
...
void UserInterface::createDebugWindow(VkRenderData& renderData) {
  ...
  ImGui::Text("Albedo");
  ImGui::Image(renderData.rdGBuffer.color.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("Depth");
  ImGui::Image(renderData.rdGBuffer.depth.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("Normal");
  ImGui::Image(renderData.rdGBuffer.normal.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("Selection");
  ImGui::Image(renderData.rdSelectionImageData.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("SSAO Noise");
  ImGui::Image(renderData.rdSSAONoiseBufferData.descriptorSet, ImVec2(64, 64));

  ImGui::Text("SSAO Color");
  ImGui::Image(renderData.rdSSAOColorBufferData.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("SSAO Blur");
  ImGui::Image(renderData.rdSSAOBlurBufferData.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("Shadow Map");
  ImGui::Image(renderData.rdShadowMapCombinedDepthBufferData.descriptorSet, ImVec2(300, 300));

  ImGui::Text("Light Spheres");
  ImGui::Image(renderData.rdLightSpheresBufferData.descriptorSet, ImVec2(imageWidth, imageHeight));

  ImGui::Text("Light Sphere Shadow Map");
  ImGui::Image(renderData.rdDynamicLightCombinedShadowData.descriptorSet, ImVec2(300, 300));
...
void UserInterface::removeDescriptorSets(VkRenderData& renderData) {
  ImGui_ImplVulkan_RemoveTexture(renderData.rdGBuffer.color.descriptorSet);
  ImGui_ImplVulkan_RemoveTexture(renderData.rdGBuffer.depth.descriptorSet);
  ImGui_ImplVulkan_RemoveTexture(renderData.rdGBuffer.normal.descriptorSet);
  ImGui_ImplVulkan_RemoveTexture(renderData.rdSelectionImageData.descriptorSet);
