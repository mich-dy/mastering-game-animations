...
bool VkRenderer::init(unsigned int width, unsigned int height) {
  ...
  mModelInstCamData.micPhysicsBodyTypeMap[physicsBodyType::dynamicBody] = "Dynamic";
...
void VkRenderer::cloneInstance(std::shared_ptr<AssimpInstance> instance) {
  ...
  newInstanceSettings.isPhysicsBodyId = b3BodyId{};
...
void VkRenderer::handleMousePositionEvents(double xPos, double yPos) {
  ...
  if (mMouseMove) {
    ...
    if (mModelInstCamData.micSelectedInstance > 0) {
      ...
      // recreate moved physics object in world
...
bool VkRenderer::draw(float deltaTime) {
  if (!initDraw(deltaTime)) {
    return false;
  }

  if (!acquireDesktopImage()) {
    return false;
  }

  if (!updateLevelAndModels(deltaTime)) {
    return false;
  }
...
bool VkRenderer::initDraw(float deltaTime) {
  if (!mApplicationRunning) {
    bool result = mApplicationRunning;
    return false;
  }
...
bool VkRenderer::renderGraphics() {
  ...
  VkHelper::transitionImageLayout(mRenderData, mRenderData.rdLightSpheresBufferData.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
  VkHelper::transitionImageLayout(mRenderData, mRenderData.rdShadowMapCombinedDepthBufferData.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL);
  ...
  VkHelper::transitionImageLayout(mRenderData, mRenderData.rdLightSpheresBufferData.image, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  ...
  swapchainImageMemoryBarrier.image = mRenderData.rdSwapchainImages.at(mImageIndex);
  ...
  vkCmdPipelineBarrier(
    ...
    0,
    0, nullptr, 0, nullptr,
    1, &swapchainImageMemoryBarrier // pImageMemoryBarriers
  );
...
bool VkRenderer::presentDesktopImage() {
  ...
  presentInfo.pWaitSemaphores = &mRenderData.rdRenderSemaphores[mImageIndex];
