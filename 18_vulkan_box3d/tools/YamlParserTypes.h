...
namespace YAML {
  ...
  // physics - physics hull
  template<>
  struct convert<physicsHullType> {
    static Node encode(const physicsHullType& rhs) {
      Node node;
      node = static_cast<int>(rhs);
      return node;
    }

    static bool decode(const Node& node, physicsHullType& rhs) {
      try {
        rhs = static_cast<physicsHullType>(node.as<int>());
      } catch (...) {
        Logger::log(1, "%s warning: could not parse physics hull type, using default 'point'\n", __FUNCTION__);
        rhs = physicsHullType::cube;
      }
      return true;
    }
  };
  ...
  struct convert<physicsBodyType> {
    static Node encode(const physicsBodyType& rhs) {
      Node node;
      node = static_cast<int>(rhs);
      return node;
    }

    static bool decode(const Node& node, physicsBodyType& rhs) {
      try {
        rhs = static_cast<physicsBodyType>(node.as<int>());
      } catch (...) {
        Logger::log(1, "%s warning: could not parse physics body type, using default 'point'\n", __FUNCTION__);
        rhs = physicsBodyType::staticBody;
      }
      return true;
    }
  };
  ...
  struct convert<ModelSettings> {
    static Node encode(const ModelSettings& rhs) {
      ...
      node["physics-enabled"] = rhs.msPhysicsEnabled;
      if (rhs.msPhysicsEnabled) {
        node["physics-hull-type"] = rhs.msPhysicsHullType;
        node["physics-hull-size"] = rhs.msPhysicsHullSize;
        node["physics-hull-offset"] = rhs.msPhysicsHullOffset;
        node["physics-hull-rotation"] = rhs.msPhysicsHullRotation;
        node["physics-hull-scale"] = rhs.msPhysicsHullScale;
        node["physics-body-type"] = rhs.msPhysicsBodyType;
        node["physics-friction-coeff"] = rhs.msPhysicsFrictionCoeff;
        node["physics-restitution-coeff"] = rhs.msPhysicsRestitutionCoeff;
      }
    ...
    static bool decode(const Node& node, ModelSettings& rhs) {
      ...
      if (node["physics-enabled"]) {
        try {
          rhs.msPhysicsEnabled = node["physics-enabled"].as<bool>();
        } catch (...) {
          Logger::log(1, "%s warning: could not physics enabled status of model '%s', disabling\n", __FUNCTION__, rhs.msModelFilename.c_str());
          rhs.msPhysicsEnabled = false;
        }

        try {
          rhs.msPhysicsHullType = node["physics-hull-type"].as<physicsHullType>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics hull type, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsHullType = defaultSettings.msPhysicsHullType;
        }
        try {
          rhs.msPhysicsHullSize = node["physics-hull-size"].as<glm::vec3>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics hull size, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsHullSize = defaultSettings.msPhysicsHullSize;
        }
        try {
          rhs.msPhysicsHullOffset = node["physics-hull-offset"].as<glm::vec3>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics hull offset, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsHullOffset = defaultSettings.msPhysicsHullOffset;
        }
        try {
          rhs.msPhysicsHullRotation = node["physics-hull-rotation"].as<glm::vec3>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics hull rotation, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsHullRotation = defaultSettings.msPhysicsHullRotation;
        }
        try {
          rhs.msPhysicsHullScale = node["physics-hull-scale"].as<glm::vec3>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics hull scale, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsHullScale = defaultSettings.msPhysicsHullScale;
        }
        try {
          rhs.msPhysicsBodyType = node["physics-body-type"].as<physicsBodyType>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics body type, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsBodyType = defaultSettings.msPhysicsBodyType;
        }
        try {
          rhs.msPhysicsFrictionCoeff = node["physics-friction-coeff"].as<float>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics friction coefficient, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsFrictionCoeff = defaultSettings.msPhysicsFrictionCoeff;
        }
        try {
          rhs.msPhysicsRestitutionCoeff = node["physics-restitution-coeff"].as<float>();
        } catch (...) {
          Logger::log(1, "%s warning: could not parse physics restitution coefficient, init with a default value\n", __FUNCTION__);
          rhs.msPhysicsRestitutionCoeff = defaultSettings.msPhysicsRestitutionCoeff;
        }
      }
      return true;
    }
