...
YAML::Emitter& operator<<(YAML::Emitter& out, const physicsHullType& hullType) {
  out << YAML::Flow;
  out << static_cast<int>(hullType);
  return out;
}

YAML::Emitter& operator<<(YAML::Emitter& out, const physicsBodyType& bodyType) {
  out << YAML::Flow;
  out << static_cast<int>(bodyType);
  return out;
}
...
YAML::Emitter& operator<<(YAML::Emitter& out, const ModelSettings& settings) {
  ...
  out << YAML::Key << "physics-enabled";
  out << YAML::Value << settings.msPhysicsEnabled;
  if (settings.msPhysicsEnabled) {
    out << YAML::Key << "physics-hull-type";
    out << YAML::Value << settings.msPhysicsHullType;
    out << YAML::Key << "physics-hull-size";
    out << YAML::Value << settings.msPhysicsHullSize;
    out << YAML::Key << "physics-hull-offset";
    out << YAML::Value << settings.msPhysicsHullOffset;
    out << YAML::Key << "physics-hull-rotation";
    out << YAML::Value << settings.msPhysicsHullRotation;
    out << YAML::Key << "physics-hull-scale";
    out << YAML::Value << settings.msPhysicsHullScale;
    out << YAML::Key << "physics-body-type";
    out << YAML::Value << settings.msPhysicsBodyType;
    out << YAML::Key << "physics-friction-coeff";
    out << YAML::Value << settings.msPhysicsFrictionCoeff;
    out << YAML::Key << "physics-restitution-coeff";
    out << YAML::Value << settings.msPhysicsRestitutionCoeff;
  }
...
float YamlParser::getNearPlane() {
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "near-clip-plane") {
        plane = it->second.as<float>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
float YamlParser::getFarPlane() {
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "far-clip-plane") {
        plane = it->second.as<float>();
      }
    }
  } catch (...) {
...
float YamlParser::getOrthoNearFarPlane() {
  float plane = 40.0f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return plane;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ortho-nearfar-clip-plane") {
        plane = it->second.as<float>();
      }
    }
  } catch (...) {
...
bool YamlParser::getSSAOEnabled() {
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ssao-enabled") {
        ssaoEnabled = it->second.as<bool>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
float YamlParser::getSSAORadius() {
  float radius = 5.0f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return radius;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ssao-radius") {
        radius = it->second.as<float>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
float YamlParser::getSSAOBias() {
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ssao-bias") {
        bias = it->second.as<float>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
int YamlParser::getSSAOExponent() {
  int exponent = 4;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return exponent;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ssao-exponent") {
        exponent = it->second.as<int>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
bool YamlParser::getSSAOBlurEnabled() {
  bool ssaoBlurEnabled = false;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return ssaoBlurEnabled;
  }

  YAML::Node settingsNode = mYamlNode["settings"];
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ssao-blur-enabled") {
        ssaoBlurEnabled = it->second.as<bool>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
int YamlParser::getSSAOBlurRadius() {
  int radius = 3;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return radius;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "ssao-blur-radius") {
        radius = it->second.as<int>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
bool YamlParser::getShadowMapEnabled() {
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-enabled") {
        shadowMapEnabled = it->second.as<bool>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
float YamlParser::getShadowMapSplitLambda() {
  float lambda = 0.95f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return lambda;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-split-lambda") {
        lambda = it->second.as<float>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
float YamlParser::getShadowMapConstantDepthBias() {
  float bias = 1.25f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return bias;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-constant-bias") {
        bias = it->second.as<float>();
      }
    }
  } catch (...) {
...
float YamlParser::getShadowMapSlopeDepthBias() {
  float bias = 1.75f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return bias;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-slope-bias") {
        bias = it->second.as<float>();
      }
    }
  } catch (...) {
...
bool YamlParser::getShadowMapPFCEnabled() {
  bool shadowMapPCFEnabled = false;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return shadowMapPCFEnabled;
  }

  YAML::Node settingsNode = mYamlNode["settings"];
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-pcf-enabled") {
        shadowMapPCFEnabled = it->second.as<bool>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
float YamlParser::getShadowMapPCFScale() {
  float scale = 0.1f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return scale;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-pcf-scale") {
        scale = it->second.as<float>();
      }
    }
  } catch (...) {
...
int YamlParser::getShadowMapPCFRange() {
  int range = 3;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return range;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "shadowmap-pcf-range") {
        range = it->second.as<int>();
      }
    }
  } catch (...) {
    Logger::log(1, "%s error: could not parse file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
...
bool YamlParser::createConfigFile(VkRenderData &renderData, ModelInstanceCamData modInstCamData) {
  ...
  mYamlEmit << YAML::BeginMap;
  mYamlEmit << YAML::Key << "near-clip-plane";
  ...
  mYamlEmit << YAML::Key << "ssao-enabled";
  mYamlEmit << YAML::Value << renderData.rdEnableSSAO;
  mYamlEmit << YAML::Key << "ssao-radius";
  mYamlEmit << YAML::Value << renderData.rdSSAORadius;
  mYamlEmit << YAML::Key << "ssao-bias";
  mYamlEmit << YAML::Value << renderData.rdSSAOBias;
  mYamlEmit << YAML::Key << "ssao-exponent";
  mYamlEmit << YAML::Value << renderData.rdSSAOExponent;
  mYamlEmit << YAML::Key << "ssao-blur-enabled";
  mYamlEmit << YAML::Value << renderData.rdEnableSSAOBlur;
  mYamlEmit << YAML::Key << "ssao-blur-radius";
  mYamlEmit << YAML::Value << renderData.rdSSAOBlurRadius;
  mYamlEmit << YAML::Key << "shadowmap-enabled";
  mYamlEmit << YAML::Value << renderData.rdEnableShadowMap;
  mYamlEmit << YAML::Key << "shadowmap-split-lambda";
  mYamlEmit << YAML::Value << renderData.rdShadowMapSplitLambda;
  mYamlEmit << YAML::Key << "shadowmap-constant-bias";
  mYamlEmit << YAML::Value << renderData.rdShadowMapConstantDepthBias;
  mYamlEmit << YAML::Key << "shadowmap-slope-bias";
  mYamlEmit << YAML::Value << renderData.rdShadowMapSlopeDepthBias;
  mYamlEmit << YAML::Key << "shadowmap-pcf-enabled";
  mYamlEmit << YAML::Value << renderData.rdEnableShadowMapPCF;
  mYamlEmit << YAML::Key << "shadowmap-pcf-scale";
  mYamlEmit << YAML::Value << renderData.rdShadowMapPCFScale;
  mYamlEmit << YAML::Key << "shadowmap-pcf-range";
  mYamlEmit << YAML::Value << renderData.rdShadowMapPCFRange;
  mYamlEmit << YAML::EndMap;
  mYamlEmit << YAML::EndMap;
